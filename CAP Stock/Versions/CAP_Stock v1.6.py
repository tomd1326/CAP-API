...
async def fetch_with_retries(payload, registration, mileage_for_request, capid, reg_date, session, valuation_date_type, round_to):
        ...
        if response.status == 200:
            ...
            if valuation is not None:
                ...
                if not clean and round_to == 1000:
                    ...
                    return await fetch_with_retries(payload, registration, mileage_for_request, capid, reg_date, session, valuation_date_type, 10000)
...
async def process_row(idx, row, df, session):
    ...
    task1 = asyncio.create_task(
        fetch_with_retries(current_payload, registration, rounded_mileage, capid, reg_date, session, 'current', 1000)
    )
    task2 = asyncio.create_task(
        fetch_with_retries(fixed_payload, registration, rounded_mileage, capid, reg_date, session, 'fixed', 1000)
    )
