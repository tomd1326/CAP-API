...
# Function to remove text after "For only £99" in ExtrasSpec column
def remove_text_after_string(text):
    if isinstance(text, str):  # Check if the value is a string
        match = re.search(r'For only £99', text)
        if match:
...
# Extract the date and time from the original input file's name
original_datetime = re.search(r'\d{14}', input_excel_path)
if original_datetime:
    original_datetime = original_datetime.group()
else:
    original_datetime = "unknown_datetime"
...
output_csv_path = input_excel_path.replace(original_datetime, f'_{current_date}')
