...
# Constructing the paths using the home directory
cap_config_path = os.path.join(home_dir, 'OneDrive - Motor Depot', 'Python Scripts', 'CAP')
log_dir = os.path.join(home_dir, 'OneDrive - Motor Depot', 'Python Scripts', 'CAP', 'CAP Pricing', 'Logs')
input_dir = os.path.join(home_dir, 'OneDrive - Motor Depot', 'Python Scripts', 'CAP', 'CAP Pricing')
output_dir = os.path.join(home_dir, 'OneDrive - Motor Depot', 'Python Scripts', 'CAP', 'CAP Pricing', 'Outputs')
...
async def process_row(session, row, total_valid_rows):
    ...
    # Fetch current valuation from LIVE_URL
    live_data = await fetch_and_parse_data(session, LIVE_URL, live_payload)
    ...
    # Fetch old valuation from LIVE_URL
    live_payload['valuationDate'] = FIXED_VALUATION_DATE
    live_old_data = await fetch_and_parse_data(session, LIVE_URL, live_payload)
    ...
    return [row['VRM'], clean_month, retail_month, clean_live, retail_live, capid_data['CAPMan'], capid_data['CAPMod'], capid_data['CAPDer'], row['DFR'],
            row['CAPID'], row['Mileage'], VALUATION_DATE, FIXED_VALUATION_DATE]



# Prepare for output CSV with updated headers
output_header = [
    "VRM", "Clean_Month", "Retail_Month", "Clean_Live", "Retail_Live", "CAPID", "CAPMan", "CAPMod", "CAPDer", "DFR", "Mileage", "Live_Date", "Month_Date"
]
